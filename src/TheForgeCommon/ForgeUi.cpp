...
void pushMouseDown(const float x, const float y) { gMouse.pushDown(x, y); }

void pushMouseUp(const float x, const float y) { gMouse.pushUp(x, y); }
...
DragState drag() { return gMouse.drag(); }

Drop readDrop() { return gMouse.readDrop(); }
